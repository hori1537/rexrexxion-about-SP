...
raw_input = pandas.read_csv(open(str(ADDRESS)+str(CSV_NAME_POIS)))
#raw_input = numpy.loadtxt(open(str(ADDRESS)+str(CSV_NAME_POIS)), delimiter=",",skiprows=1)
...
TRAIN_DATA_SIZE=DATA_SIZE_POIS-10
#DATE_SIZE_POIS =2448
...
model = Sequential()
model.add(Dense(DATA_SIZE_POIS, input_dim=62, activation='relu'))
...
model.save('C:\Deeplearning/model_POIS_' +str(score[0]) + '.h5')
...
print('C:\Deeplearning/model_POIS_' +str(score[0]) + '.h5')